...
describe('AdminCompanyDetailComponent', () => {
  ...
  it('shows the first five Northline admins, then +1 more', () => {
    const req = httpMock.expectOne(`${environment.api_url}/admin/companies/CO-20`);
    req.flush('missing', { status: 404, statusText: 'Not Found' });
    fixture.componentInstance.detail = fixtureCompanyDetail('CO-22', new Date('2026-09-24T04:00:00.000Z'));
    fixture.componentInstance.showAllAdmins = false;
    fixture.componentInstance.tab = 'overview';
    fixture.detectChanges();

    const text = fixture.nativeElement.textContent;
    const shown = fixture.nativeElement.querySelectorAll('.admin-admin-list li');
    ...
    const more = fixture.nativeElement.querySelector('.admin-admin-more');
    expect(more.textContent.trim()).toBe('+1 more');

    more.click();
    fixture.detectChanges();
    expect(fixture.nativeElement.textContent).toContain('Admin 6');
    expect(fixture.nativeElement.querySelector('.admin-admin-more')).toBeNull();
  });
});
